...
async function testModels() {
    try {
        ...
        const lesson1 = await Node.create({
            title: "Hello World",
            type: "lesson",
        ...
        const lesson2 = await Node.create({
            title: "Variables",
            type: "lesson",
        ...
        const challenge = await Node.create({
        ...
        for (const sec of sections) {
            ...
            for (const chap of chapters) {
                ...
                const nodes = await Node.find({ chapterId: chap._id }).sort({ order: 1 });
